...
class KitchenSchedules(MethodView):
  @blueprint.arguments(GetKitchenScheduleParamaters, location='query')
  ...
  def get(self, parameters):
    ...
    if in_progress is not None:
      if in_progress:
        query_set = filter(lambda s: s['status'] == 'progress', schedules)
      else:
        query_set = filter(lambda s: s['status'] != 'progress', schedules)
    since = parameters.get('since')
    if since is not None:
      query_set = filter(lambda s: s['scheduled'] >= since, schedules)
